...
let menuPrincipal = async (cliente) => {
  try {
    ...
    console.error("Error en menuPrincipal: ", error)
  }
}
// Función Cambiar clave
let cambiarClave = async (cliente) => {
  try {
    ...
    formPassword.addEventListener("submit", async (evt) => {
      evt.preventDefault()

      let password = parseInt(input.value)
      if (password) {
        cliente.password = password
        await guardarDatos(cliente)
        ...
        return cliente
      } else console.error("No es posible realizar la operación")
    ...
    throw new Error("Error en cambiarClave: ", error)
  }
}

let guardarDatos = async (cliente) => {
...
let consultarSaldo = (cliente) => {
    ...
    throw new Error ("Error en consultarSaldo: " + error)
  }
}

let girarRap = async (cliente, giro) => {
  try {
    ...
    throw new Error("Error en GirarRap: ", error)
  }
}

let girarOtro = async (cliente, giro) => {
  try {
    let { saldo } = cliente?.cuentaCorr ?? {}
    ...
    formGiro.addEventListener("submit", async (evt) => {
      evt.preventDefault()
    ...
    throw new Error("Error en GirarOtro: ", error)
  }
}
